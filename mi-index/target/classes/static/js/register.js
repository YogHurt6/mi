...
//验证码倒计时
function setReminTime(time){
	if(time == 0){
		$('#getCode').removeAttr('disabled');
		$('#getCode').value = "重新发送" ;
	}else{
		time--;
		$('#getCode').value="重新发送(" + time + ")";
	}
}
...
function sendCode(){
	...
	setTimeout(function(){setReminTime(60);}, 1000);
	
	//请求方式  地址  参数  回调函数
	// TODO: 发送请求
	
	$.post("emailcode", {email:email}, result=>{
		if(result == 200){
			alert("验证码已发送");
		}
	}, "json");
...
function checkRegister(){
	...
	var pwd = $.trim($("#nickname").val());
	var tel = $("#tel").val();
	
	$.post("member", {op: "register", nickName:nickName, email:email, pwd:pwd, tel:tel, code:code}, result =>{
		if(result.data.code == 200){
			showMsg("注册成功，即将进入登陆界面");
